...
if st.sidebar.button("Simpan & Tampilkan Nota"):
    if not pembeli or len(st.session_state["items"]) == 0:
        ...
        for it in st.session_state["items"]:
            ...
            barang_ditampilkan = f"{it['barang']} {rincian_kurung}".strip()
        ...
        for i, row in df_nota.iterrows():
            ...
            nama_barang_wrapped = textwrap.wrap(row["Barang"], width=40)
            for wrap_line in nama_barang_wrapped:
                draw.text((100, y), wrap_line, font=font, fill="black")
                y += line_height
            y -= line_height
